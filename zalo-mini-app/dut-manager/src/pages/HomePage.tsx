...
const HomePage: React.FC = () => {
  ...
  const menuItems = [
    { id: 1, label: 'Báo cáo', icon: iconTuVan, path: PATHS.ACADEMIC_REPORTS },
    { id: 2, label: 'Lịch họp', icon: iconTraCuu, path: PATHS.MEETINGS },
    { id: 3, label: 'Đơn phép', icon: iconXeBus, path: PATHS.PERMISSIONS },
    { id: 4, label: 'Bài tập', icon: iconGiaoTrinh, path: PATHS.HOMEWORKS },
    { id: 5, label: 'Tất cả', icon: iconTatCa, path: PATHS.SHORTCUTS },
  ];
    ...
    <Page className="bg-surface">
      ...
      <div className="px-4 pt-3 pb-20 flex flex-col gap-3">
        ...
        <div className="flex flex-row items-center py-2 pl-4 pr-2 gap-2 bg-white rounded-[30px]">
          ...
          <input
            type="text"
            placeholder="Tìm tính năng, báo cáo, lịch họp..."
        ...
        <div className="flex flex-row gap-3">
          <div
            onClick={() => navigateForward(navigate, PATHS.REPORTS)}
            className="flex-1 bg-white rounded-xl p-3 flex flex-col gap-3 cursor-pointer"
          >
            <img src={cardTraCuuHoSo} alt="Báo cáo & Nhiệm vụ" className="w-8 h-8 object-contain" />
            <div className="flex flex-col gap-1">
              <span className="font-bold text-body-lg text-dark">Nhiệm vụ & Báo cáo</span>
              <span className="text-body-sm text-muted">Học tập, bài tập, đơn phép</span>
            </div>
          </div>
          <div
            onClick={() => navigateForward(navigate, PATHS.CALENDAR)}
            className="flex-1 bg-white rounded-xl p-3 flex flex-col gap-3 cursor-pointer"
          >
            <img src={cardThongBao} alt="Lịch & Sự kiện" className="w-8 h-8 object-contain" />
            <div className="flex flex-col gap-1">
              <span className="font-bold text-body-lg text-dark">Lịch & Sự kiện</span>
              <span className="text-body-sm text-muted">Lịch họp lab, sự kiện</span>
            </div>
          </div>
        </div>
