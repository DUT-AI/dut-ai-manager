...
export const apiKeyService = {
    create: async (data: CreateApiKeyDto): Promise<ApiResponse<RoleApiKeySecret>> => {
        const response = await axiosInstance.post<ApiResponse<RoleApiKeySecret>>('/rbac/api-keys', data);
        return response.data;
    },

    getByRole: async (roleId: number): Promise<ApiResponse<RoleApiKey[]>> => {
        const response = await axiosInstance.get<ApiResponse<RoleApiKey[]>>(`/rbac/api-keys/role/${roleId}`);
        return response.data;
    },

    revoke: async (keyId: number): Promise<ApiResponse<null>> => {
        const response = await axiosInstance.delete<ApiResponse<null>>(`/rbac/api-keys/${keyId}`);
        return response.data;
    },
};
