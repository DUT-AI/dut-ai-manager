...
def upgrade() -> None:
    ...
    sa.Column('status', sa.Enum('NOT_JOINED', 'JOINED', name='participantstatus'), nullable=False),
    sa.Column('link_image', sqlmodel.sql.sqltypes.AutoString(length=500), nullable=True),
    sa.ForeignKeyConstraint(['created_by'], ['users.id'], ),
    ...
    op.create_index(op.f('ix_meeting_participants_created_at'), 'meeting_participants', ['created_at'], unique=False)
