...
def seed_mock_data():
    with Session(engine) as session:
        ...
        mock_users = [
            ...
            {
                "name": "Lê Văn C (Inactive)",
                "email": "levanc@gmail.com",
                "phone_number": "0112233445",
            },  # This user will have no activity
        ]
        ...
        if True:
            ...
            m_may_1 = create_meeting(
                last_month,
                last_month_year,
                10,
                "Buổi sinh hoạt tháng " + str(last_month) + " - 1",
            )
            m_may_2 = create_meeting(
                last_month,
                last_month_year,
                20,
                "Buổi sinh hoạt tháng " + str(last_month) + " - 2",
            )

            # Meetings for current month
            m_june_1 = create_meeting(
                current_month,
                current_year,
                5,
                "Buổi sinh hoạt tháng " + str(current_month) + " - 1",
            )
            ...
            def add_participant(user_id, meeting, checkin_offset=0):
                ...
                checkin = meeting.start_time + datetime.timedelta(
            ...
            v1 = ViolationModel(
                user_id=user_b.id, date=m_may_1.start_time, reason="Đi trễ"
            )
            b1 = BonusPointModel(
                user_id=user_a.id,
                points=10,
                reason="Phát biểu tích cực",
                date=m_may_1.start_time,
            )
            b2 = BonusPointModel(
                user_id=user_b.id,
                points=5,
                reason="Làm bài tập tốt",
                date=m_may_2.start_time,
            )
            b3 = BonusPointModel(
                user_id=user_a.id,
                points=15,
                reason="Hỗ trợ tech",
                date=m_june_1.start_time,
            )

            session.add_all([v1, b1, b2, b3])

            # Monthly stats
            stat_a_may = MonthlyUserStatsModel(
                user_id=user_a.id,
                month=last_month,
                year=last_month_year,
                total_activity_hours=4.0,
                total_bonus_points=10,
                violation_count=0,
                assigned_title=UserTitle.ACTIVE.value,
            )
            stat_b_may = MonthlyUserStatsModel(
                user_id=user_b.id,
                month=last_month,
                year=last_month_year,
                total_activity_hours=3.6,
            ...
            stat_a_june = MonthlyUserStatsModel(
                user_id=user_a.id,
                month=current_month,
                year=current_year,
                total_activity_hours=2.0,
                total_bonus_points=15,
                violation_count=0,
            )
            stat_b_june = MonthlyUserStatsModel(
                user_id=user_b.id,
                month=current_month,
                year=current_year,
            ...
            stat_c_may = MonthlyUserStatsModel(
                user_id=user_c.id,
                month=last_month,
                year=last_month_year,
                total_activity_hours=0.0,
            ...
            stat_c_june = MonthlyUserStatsModel(
                user_id=user_c.id,
                month=current_month,
                year=current_year,
                total_activity_hours=0.0,
