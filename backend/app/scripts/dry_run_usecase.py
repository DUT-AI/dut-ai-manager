...
from app.homework.infrastructure.repository import HomeworkSubmissionRepository, HomeworkRepository
...
class DryRunEventBus:
    ...
    async def publish(self, event):
        ...
        print(f"WOULD PUBLISH VIOLATION: user={event.user_id}, hw={event.homework_id}, reason='{event.reason}'")
...
async def main():
    async for session in get_db():
        ...
        postpone_requests = perm_repo.get_postpone_requests_for_homeworks(homework_ids, owner_ids)
        print(f"Fetched {len(postpone_requests)} postpone requests matching these HWs and Users.")
        ...
        for sub in overdue:
            if sub.owner_id in [13, 15, 12]:
                print(f"Processing target user {sub.owner_id} - hw {sub.homework_id}...")
