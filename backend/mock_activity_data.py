...
def seed_mock_data():
    with Session(engine) as session:
        ...
        session.execute(
            text("TRUNCATE TABLE monthly_user_stats RESTART IDENTITY CASCADE;")
        )
        ...
        session.execute(
            text("TRUNCATE TABLE meeting_participants RESTART IDENTITY CASCADE;")
        )
        ...
        mock_users = session.scalars(
            select(UserModel).where(UserModel.email.like("mock_user_%"))
        ).all()
        ...
        while current_date <= end_date:
            if current_date.weekday() in [1, 4]:  # Tue, Fri
                date_time = datetime.datetime(
                    current_date.year, current_date.month, current_date.day, 18, 0, 0
                )
                m = Meeting(
            ...
            for m in meetings:
                if profile == "ACTIVE":
                    if random.random() < 0.95:  # 95% attendance
                        # Check in on time
                        cin = m.start_time - datetime.timedelta(
                    ...
                    if random.random() < 0.7:  # 70% attendance
                        ...
                        cin = (
                            m.start_time
                            + datetime.timedelta(minutes=random.randint(10, 30))
                            if late
                            else m.start_time
                            - datetime.timedelta(minutes=random.randint(0, 10))
                        )
                        cout = m.end_time
                        session.execute(
                            text("""
                        ...
                        if late:
                            v = ViolationModel(
                                user_id=user.id, date=m.start_time, reason="Đi trễ"
                            )
                            session.add(v)
                    else:
                        v = ViolationModel(
                            user_id=user.id, date=m.start_time, reason="Vắng"
                        )
                        session.add(v)
                elif profile == "POOR":
        ...
        from app.user.infrastructure.monthly_stats_repository import (
            MonthlyUserStatsRepository,
        )
        ...
        assign_uc = AssignMonthlyTitlesUseCase(
        ...
        logger.success(
            "Mock data (50 users) generated and titles assigned successfully!"
        )
