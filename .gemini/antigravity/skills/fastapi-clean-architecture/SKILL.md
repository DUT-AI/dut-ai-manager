...
2. **Domain Layer** (`src/domain/`)
   - **Entities**: Business entities and value objects (pure logic)
   - **Use Cases**: Specific business actions (Interactors) - coordination logic
   - **Interfaces**: Abstract definitions for repositories and external services
...
- Use Cases depend on Entities and Interfaces (not implementations).
- Infrastructure implements Domain interfaces.
- Domain (Entities/Use Cases) NEVER imports from API or Infrastructure.
- Use dependency injection to wire implementations to interfaces.
